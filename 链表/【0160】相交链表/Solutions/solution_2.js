...
var getIntersectionNode = function (headA, headB) {
  ...
  /**
   * 思路：双指针，第一个指针从 headA 开始，第二个指针从 headB 开始
   * 依次往前走，当指针走到尾部时，则指向另一个链表的开头位置，继续往前走
   * 当两个指针相遇的地方，就是交点
   */
  ...
  while (first !== second) {
    ...
    // 如果第一个指针为 null 了，判断当前是否已经交换到了第二个链表上
    // 如果没交换过，则交换到第二个链表上
    // 否则直接返回 null，查找结束
    if (first === null) {
      if (!isSkipFirst) {
    ...
    // 如果第二个指针为 null 了，判断当前是否已经交换到了第一个链表上
    // 如果没交换过，则交换到第一个链表上
    // 否则直接返回 null，查找结束
    if (second === null) {
      if (!isSkipSecond) {
  ...
  // 执行到这里的条件是 frist === second，即两个指针相等
  return first;
};
// @lc code=end
